...
public class Left3Note {
  ...
  public static Command getCommand() {
    ...
    return Commands.runOnce(() -> Subsystems.m_driveSubsystem.resetPose(trajectory1.getInitialPose()))
        .andThen(Commands.runOnce(()->Subsystems.m_armSubsystem.armMotor.set(0)))
        .andThen(new AutonShooterCommand(-2400,-2400).deadlineWith(new AutonSlackinatorCommand()))
      ...
      // .andThen(Commands.runOnce(()->Subsystems.m_shooterSubsystem.intake(0)))
        .andThen(Subsystems.m_driveSubsystem.getRamseteCommand(trajectory1).alongWith(Commands.waitSeconds(10).raceWith(new AutonIntakeCommand())).alongWith(new AutonShooterCommand(Constants.Auton.subwooferTopRPM, Constants.Auton.subwooferBottomRPM)))
      ...
      // .andThen(Commands.runOnce(()->Subsystems.m_shooterSubsystem.convey(0)))
        ...
        .andThen(new AutonArmCommand(-4, false))
        // .andThen(new AutonShooterCommand(Constants.Auton.subwooferTopRPM, Constants.Auton.subwooferBottomRPM))
        .andThen((Commands.waitSeconds(Constants.Auton.feedTime).deadlineWith(new AutonConveyorCommand())).deadlineWith(new AutonArmCommand(-4, true)))
        .andThen(Commands.runOnce(()->Subsystems.m_shooterSubsystem.updateHasPassed()))
       ...
       // .andThen(Commands.runOnce(()->Subsystems.m_shooterSubsystem.convey(0)))
        // .andThen(Commands.runOnce(()->Subsystems.m_shooterSubsystem.zeroMotors())).alongWith(Commands.runOnce(()->Subsystems.m_shooterSubsystem.convey(0)))
        .andThen(new AutonArmCommand(1.0, false))
        // .andThen(new AutonShooterCommand(Constants.Auton.subwooferTopRPM, Constants.Auton.subwooferBottomRPM))
        .andThen((Commands.waitSeconds(Constants.Auton.feedTime).deadlineWith(new AutonConveyorCommand())).deadlineWith(new AutonArmCommand(1.0, true)))
        .andThen(Commands.runOnce(()->Subsystems.m_shooterSubsystem.updateHasPassed()))
        .andThen(Commands.runOnce(()->Subsystems.m_shooterSubsystem.conveyerMotor.set(0)))
