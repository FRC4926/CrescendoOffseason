...
public class VisionCommand extends Command {
  double turnEffort;
  double driveEffort;
  GalacPIDController alignPidController = new GalacPIDController(0.0145, 0, 0, 0.2, () -> RobotContainer.Subsystems.m_limelightSubsystem.getX(), 0, 1);
  GalacPIDController alignPidControllerG = new GalacPIDController(0.02, 0, 0, 0, () -> RobotContainer.Subsystems.m_driveSubsystem.getGyroYaw()%360, -30, 1);
  ...
  public void execute() {
    ...
    SmartDashboard.putNumber("MotorEffortFrontleft", RobotContainer.Subsystems.m_driveSubsystem.frontLeftMotor.get());
    ...
    if (RobotContainer.Controllers.m_driverController.getAButton()) {
        ...
        if(Math.abs(Subsystems.m_limelightSubsystem.getX())<6){
          for (var i = 0; i < 15; i++) {
           Subsystems.m_shooterSubsystem.m_ledBuffer.setRGB(i,255,165,10);
          }
        }
