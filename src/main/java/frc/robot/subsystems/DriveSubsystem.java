...
public class DriveSubsystem extends SubsystemBase {
  ...
  public Trajectory getTrajectory(String filePath) {
    //SmartDashboard.putString("Auton Path", "paths/" + RobotContainer.m_alliance.getSelected() + filePath + ".wpilib.json");
    try {
        Path path = Filesystem.getDeployDirectory().toPath().resolve("paths/" + "Blue" + filePath + ".wpilib.json");
    ...
    try {
      return TrajectoryUtil.fromPathweaverJson(Filesystem.getDeployDirectory().toPath().resolve("paths/" +filePath + ".wpilib.json"));
    } catch (IOException e) {
      e.printStackTrace();
    }
    try {
